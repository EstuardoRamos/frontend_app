...
// Si ya tienes ClientesService, úsalo; si no, simula opciones.
...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, FormsModule, TableModule, ToolbarModule, ButtonModule, RippleModule, ToastModule, ConfirmDialogModule, InputTextModule, InputNumberModule, DialogModule, TagModule, InputIconModule, IconFieldModule, SelectModule],
...
export class ReservasCrudComponent implements OnInit {
    ...
    // acepta CF o NIT básico (ajusta si necesitas SAT exacto)
    ...
    ngOnInit(): void {
        ...
        // TODO: sustituir por tu ClientesService real
        // Ejemplo placeholder:
        this.clientesOptions = [
            // { label: 'Juan Pérez', value: 'uuid-juan' },
            // ...
        ];
    }
    ...
    save() {
        ...
        this.reservasSvc.crear(dto).subscribe({
            next: (_) => {
                this.dialogVisible = false;
                this.load();
                this.toast.add({ severity: 'success', summary: 'Reserva creada' });
            },
    ...
    doCheckin(r: ReservaDTO) {
        this.reservasSvc.checkin(r.id).subscribe({
            next: (res) => {
                this.replaceRow(res);
                this.toast.add({ severity: 'success', summary: 'Check-in realizado' });
            },
    ...
    doCheckout(r: ReservaDTO) {
        this.reservasSvc.checkout(r.id).subscribe({
            next: (res) => {
                this.replaceRow(res);
                this.toast.add({ severity: 'success', summary: 'Check-out realizado' });
                this.openFacturaDialog(res);
            },
        ...
        this.confirm.confirm({
            ...
            accept: () => {
                this.reservasSvc.cancelar(r.id).subscribe({
                    next: (res) => {
                        this.replaceRow(res);
                        this.toast.add({ severity: 'success', summary: 'Reserva cancelada' });
                    },
    ...
    mapHabitacion(id: string) {
        return this.cacheHabitaciones.get(id) || id;
    }
    mapCliente(id: string) {
        return this.cacheClientes.get(id) || id;
    } // llena cache cuando tengas ClientesService
  ...
  this.invoice = {
    ...
    serie: 'A',
    clienteNit: 'CF',
    clienteNombre: this.mapCliente(r.clienteId) || '' // si tienes el nombre
  };
...
emitirFactura() {
  ...
  if (!this.nitRegex.test(inv.clienteNit)) {
    this.toast.add({severity:'warn', summary:'Validación', detail:'NIT inválido (usa CF o NIT válido).'});
    return;
  }
  this.facturasSvc.emitir(inv as any).subscribe({
    next: (f: FacturaDTO) => {
      ...
      // opcional: refrescar reservas o abrir visor de factura
    },

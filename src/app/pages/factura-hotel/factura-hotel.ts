// src/app/pages/facturas-hotel/facturas-hotel.component.ts
...
@Component({
  ...
  imports: [
    ...
    ToolbarModule, TableModule, SelectModule, DatePickerModule, TagModule,
    ButtonModule, ToastModule, DialogModule, InputTextModule, InputIconModule,
    IconFieldModule, MultiSelectModule
  ],
  template: `
    <p-toolbar styleClass="mb-4">
      <ng-template #start>
        <div class="flex flex-wrap items-end gap-3">
          <p-select class="w-20rem" [(ngModel)]="hotelId" [options]="hotelesOptions"
            optionLabel="label" optionValue="value" placeholder="Selecciona hotel"
            (onChange)="load()" />

          <div class="flex flex-col">
            <label class="text-sm mb-1">Rango de fechas</label>
            <p-datepicker [(ngModel)]="fechaRango" selectionMode="range" inputId="dp"
              placeholder="Desde - Hasta" (onSelect)="applyFilters()"
              dateFormat="yy-mm-dd" styleClass="w-20rem" />
          </div>

          <div class="flex flex-col">
            <label class="text-sm mb-1">Estado</label>
            <p-multiSelect [(ngModel)]="filtros.estado" [options]="estadosOptions"
              optionLabel="label" optionValue="value" defaultLabel="Todos"
              display="chip" (onChange)="applyFilters()"> </p-multiSelect>
          </div>
    ...
    <p-table [value]="rows" [paginator]="true" [rows]="10" [rowHover]="true"
    ...
    <!-- Detalle -->
    <p-dialog [(visible)]="detailVisible" [modal]="true" [style]="{ width: '760px' }" header="Detalle de factura">
      <ng-template #content>
        <div *ngIf="detalle" class="flex flex-col gap-3">
          <div class="surface-card p-3 border-round shadow-1">
            <div class="flex justify-between items-center mb-2">
              <div>
                <div class="text-900 text-lg font-bold">
                  Factura <b>{{ detalle.serie }}-{{ detalle.numero }}</b>
                </div>
            ...
            <!-- Código para valorar su estancia -->
            <div class="mt-3 p-3 border rounded-lg bg-surface-50">
              <div class="text-sm opacity-70 mb-1">Código para valorar su estancia</div>
              <div class="font-semibold break-all">{{ detalle.id }}</div>
              
            </div>

            <div class="grid mt-3">
              <div class="col-12 md:col-4">Moneda: <b>{{ detalle.moneda }}</b></div>
              <div class="col-12 md:col-4">Impuesto: <b>{{ detalle.impuesto | number: '1.2-2' }}</b></div>
              <div class="col-12 md:col-4">Propina: <b>{{ detalle.propina | number: '1.2-2' }}</b></div>
            </div>
          </div>
...
export class FacturasHotelComponent implements OnInit {
  ...
  applyFilters() {
    ...
    let d0: number | null = null, d1: number | null = null;
    ...
    this.rows = (this.allRows || []).filter((f) => {
      ...
      const byEstado = !estados.length || estados.includes(f.estado);
      const t = new Date(f.createdAt).getTime();
      const byFecha = (d0 == null && d1 == null) || (d0 != null && d1 != null && t >= d0 && t <= d1);
      return byText && byEstado && byFecha;
    });
  }

  // ====== Helpers ======
  getReviewLink(f: FacturaDTO) {
    try {
      const origin = location.origin;
      return `${origin}/reviews/hotel?facturaId=${f.id}`;
    } catch {
      return `/reviews/hotel?facturaId=${f.id}`;
    }
  }
  ...
  exportFacturaPDF(f: FacturaDTO) {
    ...
    autoTable(doc, {
      ...
      body: f.items.map(i => [
        i.descripcion,
        String(i.cantidad),
        this.asMoney(i.precioUnitario, f.moneda),
        this.asMoney(i.subtotal, f.moneda)
      ]),
      styles: { fontSize: 10 },
    ...
    // Código de valoración + link
    y += 28;
    doc.setFontSize(12);
    doc.text('Valora tu estancia:', 14, y);
    doc.setFontSize(11);
    doc.text(`Código: ${f.id}`, 14, y + 6);
    const link = this.getReviewLink(f);
    //doc.text(`URL: ${link}`, 14, y + 12);
